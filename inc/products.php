...
function get_products_recent() {
    ...
    try {
        $results = $db->query("SELECT * FROM products ORDER BY sku DESC LIMIT 0,4");
    } catch (Exception $e) {
...
function get_products_search($s) {
    ...
    try {
        $results = $db->prepare("SELECT * FROM products WHERE name LIKE ? ORDER BY sku");
 ...
 * using the order of the element in the array.
 ...
 * @return   array           the list of products that correspond to the start and end position
 */
function get_products_subset($positionStart,$positionEnd) {
    ...
    try {
        $results = $db->prepare("SELECT * FROM products ORDER BY sku ASC LIMIT ?, ?");
